...
namespace Stepper {
	...
	extern long counter_MOTOR;
	extern volatile uint32_t step_events_completed; // The number of step events executed in the current block

	FORCE_INLINE uint16_t calc_timer(unsigned short step_rate);

	//
	// Positions of stepper motors, in step units
	//
	extern volatile int32_t count_position;

	extern uint8_t last_direction_bits;        // The next stepping-bits to be output
	extern uint16_t cleaning_buffer_counter;

	extern int32_t acceleration_time;
	extern int32_t deceleration_time;
	extern uint16_t acc_step_rate; // needed for deceleration start point
	extern uint8_t step_loops, step_loops_nominal;
	extern uint16_t TIM6_ARR_nominal;

	//
	// Current direction of stepper motors (+1 or -1)
	//
	extern volatile int8_t count_direction;

	// Initialize the trapezoid generator from the current block.
	// Called whenever a new block begins.
	FORCE_INLINE void trapezoid_generator_reset();
	extern volatile int32_t endstops_trigsteps;
}
